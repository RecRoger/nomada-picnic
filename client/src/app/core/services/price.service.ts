...
import { PriceingExpenses, PriceingList, PriceingTransportation } from '@models/priceing';
import { CostsService } from '@services/costs.service';
import { ExpensesService } from '@services/expenses.service';
import { PlacesService } from '@services/places.service';
import { CostsTypes, PlacesTypes } from '@shared/enums';
import { ICost, IExpenseValue, IPlace } from '@shared/interfaces';
...
export class PriceService {
  ...
  private calculateTransportationPrice(value: BudgetData): void {
    forkJoin([
      ...
      map(([basicPlaces, publicPlaces]: IPlace[][]) => {
  ...
  private calculateExpensesPrice(guests: number): void {
    this.expensesService.getExpensesCached(Number(guests))
      .pipe(
        map((expenseValue: IExpenseValue | null) => {
